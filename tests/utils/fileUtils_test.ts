...
Deno.test("clean - removes extra files and checksums", async () => {
  await withTempDir(async (dir) => {
    ...
    assertEquals(await exists(join(dir, "remove.txt")), false, "remove.txt should be deleted");

    // Verify checksums
    assert(checksums["keep.txt"], "checksum for keep.txt should remain");
    assertEquals(checksums["remove.txt"], undefined, "checksum for remove.txt should be removed");
...
Deno.test("deleteEmptyFolders - recursive deletion", async () => {
  await withTempDir(async (dir) => {
     ...
     assertEquals(await exists(pathA), false, "a should be gone (became empty after b deleted)");
