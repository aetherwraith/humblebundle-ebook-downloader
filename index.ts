...
import { loadChecksumCache, readJsonFile } from "./utils/fileUtils.ts";
// import { formatFileSize } from "./utils/formatFileSize.ts";
import {
  checkOptions,
  parseArrayOption,
  parseIntOption,
} from "./utils/optionsUtils.ts";
import * as log from "@std/log";
import { green } from "@std/fmt/colors";

const program = new Denomander({
  app_name: "Humble Bundle Downloader",
  app_description: "Download your purchases from Humble Bundle",
  app_version: version,
});
...
program
  .globalOption("--dedup", "Dedup the downloads")
  .globalOption(
    "--bundle-folders",
    "Arrange downloads in bundle folders",
  );
...
program
  ...
  .action((bob) => log.info(JSON.stringify(program.appOptions)));
